...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{images/electrical_schematics/L298N.png}
...
In Figure~\ref{fig:h_bridge_image}, we can see an example of a single motor in an H-bridge circuit. 
...
Sensor data are usually complex, so they require a high-bandwidth and low-latency signal to transmit. In this case, Wi-Fi performs better than radio because of
the convenience and budget friendly while radio signal need separated transmitter and receiver, but the trade-off is higher latency 
...
Compared to other boards such as \href{https://docs.arduino.cc/resources/datasheets/ABX00087-datasheet.pdf}{Arduino UNO R4} or \href{https://documentation.espressif.com/0a-esp8266ex_datasheet_en.pdf}{ESP8266}.
ESP32 offers faster processing speed and more GPIO pins, allowing more sensors to be connected. It is also affordable.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth, page= 6]{images/electrical/d1_r32.png}
    ...
    \label{fig:wemos_d1_r32_pinMap}
\end{figure}



\newpage
\subsection{Temperature and humidity sensor: DHT11}
\par
\textbf{Overview}
\vspace{1em}
...
of using ultrasound are like this: you shoo`t out a sound, wait to hear it echo back, and if you have your
...
\subsection{IMU sensor: MPU 6500}
\par
The MPU-6500 is a compact 6-axis motion tracking sensor developed by \href{https://invensense.tdk.com/en-us}{TDK InvenSense}. 
It integrates three sensing components into a single chip:\verb|accelerometer, gyroscope|.This module is working precisely to measure gyro and acceleration. 
By integrated 6 axis for 2 sensors in 1 module, it helps simplify electrical system while provide the same functions that requires multiple modules. 
This combination enables full 6 degrees of freedom (6-DoF) motion tracking, making the MPU-6500 suitable for applications requiring orientation, motion detection, and navigation.


\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{images/electrical_schematics/mpu6500-block-diagram.png}
    \caption{MPU-6500 block diagram, more information can be found in this \href{https://datasheet.octopart.com/MPU-6500-InvenSense-datasheet-138896167.pdf}{datasheet}, announced by TDK InvenSense}
    \label{fig:mpu6500_diagram}
\end{figure}
...
\textbf{LED Light}
\par
LM2596 buck converter produces 3.3V that also work with ultra bright LEDs and provide solid lights system for Maven-01 to have ability to 
operate in low light condition  


\subsection{Current and voltage sensor: INA 219}
Maven-01 was designed to be teleoperated, so we need to monitor every important status like power or current
from it to know when we need to recharge or maintain it. We use INA219 sensor to measure current, voltage, by monitor 
working voltage of power source, we can know if the power is depleted or not.


\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{images/electrical_schematics/ina129_schematic.png}
    \caption{Simplified INA219 Schematic, more information can be found in this \href{https://www.ti.com/lit/ds/symlink/ina219.pdf}{datasheet}}
    \label{fig:ina219_schematic}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{images/information/ina219_spec.png}
    \caption{INA219 Specification table, it provides working current and voltage and the electrical limitations to prevent sensors damaged }
    \label{fig:ina219_specification}
\end{figure}
The INA219 is a current and power monitoring sensor used to measure current, voltage, 
and power consumption in electronic systems. The sensor operates by measuring the voltage drop across a shunt resistor using Ohm’s Law~\ref{eq:ohmslaw}. 
When current flows through the resistor, a small differential voltage is generated and measured by the INA219 to calculate the current value. 


\vspace{0.5em}
\par
In addition, the sensor measures the bus voltage of the connected power source. Based on the measured current and voltage values, the power consumption 
can also be calculated. As we mentioned before, control board is the most important part in the Maven-01 so this sensor is essential for status monitoring. Although there are
many better option for power measure voltage like MAX17043\cite{max17043} sensors which is specified manufactured for power level measurement, INA219 still a solid choice for 
tight budget and research purpose products.
