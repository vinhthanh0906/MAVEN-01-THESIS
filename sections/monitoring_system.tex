...
Through this connection, control commands and telemetry data are transmitted simultaneously using UDP packets formatted in JSON.
\vspace{1.5 em}
...
\vspace{1em}
\par 
In the previous section, Wifi \& UDP based protocol was discussed as the main protocol used for data transportation, the control communication protocol used in the MMS for MAVEN-01 was the same to it.
\vspace{1em}
If we look deeper into rover systems and many other UGVs (Unmanned Ground Vehicles), low-latency control plays a crucial role in overall performance. In MAVEN-01, Wi-Fi serves as the main communication protocol between the rover and the monitoring system, while UDP acts as the constant signal transmission protocol.
